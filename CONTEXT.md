...
- **Readable Content** is stored as article HTML and as Markdown, is shared per **Link** like the rest of Enrichment, and each form is capped at a fixed size. The Markdown is always present; the HTML form is absent when the Markdown did not come from a local parse.
...
- A page the extraction check rejects is escalated to Cloudflare only when it still carries at least the character floor of visible prose, so the metered path answers for an article the parser could not structure rather than for every video, post, and product page.
...
- **Readable Content** is not extracted by Cloudflare first; resolved: local extraction is the default because it costs CPU rather than metered, rate-limited browser time, and Cloudflare Browser Rendering is the fallback for pages already judged low-confidence, called with the page markup rather than the URL so it never pays for a second browser navigation.
...
- **Readable Content** stored from Cloudflare keeps no HTML form; resolved: the endpoint returns Markdown and there is no local artifact to re-convert, and a missing HTML form is not the flag and the row disagreeing, because a row exists exactly when the Markdown does (see ADR 0021).
